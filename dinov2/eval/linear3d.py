...
def set_global_seed(seed: int) -> None:
    ...
    Essential for 5-run statistical comparison per 3DINOv2 methodology.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def convert_to_serializable(obj):
    """Convert tensors and MONAI MetaTensors to JSON-serializable Python types."""
    if isinstance(obj, dict):
        return {k: convert_to_serializable(v) for k, v in obj.items()}
    elif isinstance(obj, (list, tuple)):
        return [convert_to_serializable(v) for v in obj]
    elif isinstance(obj, torch.Tensor):
        # Handle both regular Tensor and MONAI MetaTensor (subclass of Tensor)
        if obj.numel() == 1:
            return obj.item()
...
from dinov2.eval.metrics import MetricType, build_metric, build_classification_metrics
...
def get_args_parser(
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of evaluation periods without improvement before stopping. Set to 0 to disable.",
    )
    parser.add_argument(
        "--classifier-fpath",
        type=str,
    ...
    parser.add_argument(
        "--seed",
        type=int,
        help="global random seed for reproducibility (used for 5-run statistical analysis)",
    )
    parser.set_defaults(
        dataset_name='ICBM',
        epochs=10,
...
def evaluate_linear_classifiers(
    ...
    for i, (classifier_string, metric) in enumerate(results_dict_temp.items()):
        ...
        if (
            ...
            # Store all metrics for the best classifier
            best_metrics = {k: v.item() if hasattr(v, 'item') else v for k, v in metric.items()}

    if best_classifier_on_val is None:
        results_dict["best_classifier"] = {
            "name": best_classifier,
            "accuracy": max_accuracy,
            "metrics": best_metrics,  # All metrics: top-1, f1_macro, f1_weighted, auroc_macro, auroc_weighted
        }
    else:
        results_dict["best_classifier"] = {
            "name": best_classifier,
    ...
    if distributed.is_main_process():
        with open(metrics_file_path, "a") as f:
            # Write as single JSON line with iteration as key inside the dict
            log_entry = {"iter": iteration, **{k: convert_to_serializable(v) for k, v in results_dict.items()}}
        ...
        if eval_period > 0 and (iteration + 1) % eval_period == 0:
            ...
            if distributed.is_main_process():
                if res_dict["best_classifier"]["accuracy"] > max_val_acc:
                    ...
                    evals_without_improvement = 0
                    logger.info(f"New best val accuracy: {max_val_acc:.4f}")
                else:
                    evals_without_improvement += 1
                    logger.info(f"No improvement for {evals_without_improvement} evaluation period(s)")
            
            # Broadcast early stopping decision from rank 0
            evals_tensor = torch.tensor([evals_without_improvement], device="cuda")
            dist.broadcast(evals_tensor, src=0)
            evals_without_improvement = evals_tensor.item()
            
            # Early stopping check
            if early_stopping_patience > 0 and evals_without_improvement >= early_stopping_patience:
                logger.info(f"Early stopping triggered after {evals_without_improvement} evaluation periods without improvement")
                early_stopped = True
                break
            torch.cuda.synchronize()
...
def run_eval_linear(
    ...
    # Set global random seed for reproducibility (5-run methodology)
    ...
    # Build comprehensive metrics: accuracy, F1, and AUROC
    metric = build_classification_metrics(num_classes=num_classes, include_auc=True, include_f1=True)
    ...
    # Add additional metrics (F1, AUROC) if available
    if "metrics" in val_results_dict["best_classifier"]:
        val_metrics = val_results_dict["best_classifier"]["metrics"]
        for metric_name, metric_value in val_metrics.items():
            if metric_name != "top-1":  # top-1 already stored as accuracy
                results_dict[f"val_{dataset_name}_{metric_name}"] = 100.0 * metric_value if metric_value <= 1.0 else metric_value
    
    if "metrics" in test_results_dict["best_classifier"]:
        test_metrics = test_results_dict["best_classifier"]["metrics"]
        for metric_name, metric_value in test_metrics.items():
            if metric_name != "top-1":  # top-1 already stored as accuracy
                results_dict[f"test_{dataset_name}_{metric_name}"] = 100.0 * metric_value if metric_value <= 1.0 else metric_value
    
    logger.info("Test Results Dict " + str(results_dict))
    
    # Write final metrics to proper JSON file for batch script consumption
    metrics_json_path = os.path.join(output_dir, "metrics.json")
    with open(metrics_json_path, "w") as f:
        json.dump(results_dict, f, indent=2, default=str)
    logger.info(f"Saved final metrics to {metrics_json_path}")
