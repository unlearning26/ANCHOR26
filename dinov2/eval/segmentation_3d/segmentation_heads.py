...
class UNETRHead(nn.Module):
    def __init__(self, feature_model, input_channels, image_size, num_classes, autocast_ctx):
        ...
        # Compute layer indices for intermediate features based on model depth
        # ViT-Base: 12 blocks -> [2, 5, 8, 11]
        # ViT-Large: 24 blocks -> [5, 11, 17, 23]
        n_blocks = self.feature_model.n_blocks
        self.layer_indices = [
            n_blocks // 4 - 1,       # ~25%
            n_blocks // 2 - 1,       # ~50%
            3 * n_blocks // 4 - 1,   # ~75%
            n_blocks - 1,            # 100% (last block)
        ]
