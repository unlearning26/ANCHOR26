...
class ViTAdapter(nn.Module):
    def __init__(self, vit_model, input_channels, pretrain_size=None, conv_inplane=32, n_points=4,
        ...
        # Infer pretrain_size from model's position embedding if not provided
        if pretrain_size is None:
            # pos_embed shape: (1, n_tokens+1, embed_dim) where +1 is for CLS token
            # For 3D: n_tokens = (H/patch) * (W/patch) * (D/patch)
            n_tokens = vit_model.pos_embed.shape[1] - 1  # exclude CLS
            patch_size = vit_model.patch_size if hasattr(vit_model, 'patch_size') else 16
            # Assume cubic: n_tokens = (size/patch)^3
            tokens_per_dim = round(n_tokens ** (1/3))
            pretrain_size = tokens_per_dim * patch_size
            print(f"[ViTAdapter] Inferred pretrain_size={pretrain_size} from pos_embed (n_tokens={n_tokens})")

        self.norm_layer = partial(nn.LayerNorm, eps=1e-6)

        n_indexes = 4
        # Compute block ranges dynamically based on model depth
        # ViT-Base (12 blocks): [[0,2], [3,5], [6,8], [9,11]]
        # ViT-Large (24 blocks): [[0,5], [6,11], [12,17], [18,23]]
        num_blocks = len(self.vit_model.blocks)
        blocks_per_stage = num_blocks // n_indexes
        self.indexes = []
        for i in range(n_indexes):
            start = i * blocks_per_stage
            end = (i + 1) * blocks_per_stage - 1
