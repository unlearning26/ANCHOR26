#!/bin/bash
# Phase 1 Parallel Evaluation - avg_pool features (Setting A ONLY)
# Distributes checkpoints across 4 A100 GPUs
#
# This runs Setting A (Track A/B analysis, cross-bin transfer) but SKIPS Setting B
# For Setting B (controlled perturbation), run: ./run_perturbation_parallel_avg_pool.sh
# Usage:
#   PHASE1_PRESET=abdomenatlas_core ./run_phase1_parallel_avg_pool.sh
#   PHASE1_PRESET=totalsegmentermri_core ./run_phase1_parallel_avg_pool.sh
#   ANALYSIS_NAME=my_dataset MANIFEST=../../data_manifests/phase1_anisotropy_robustness/abdomenatlas/original_bins/manifest_sampled.json ./run_phase1_parallel_avg_pool.sh
